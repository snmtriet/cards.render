...
export default memo(function SiteChat({
  ...
  return (
    <>
      <div
        className={classNames("site-chat", { "chat--open": isShowFilter })}
        style={{ height: "100%" }}
      >
        <div className="site-chat__container">
          <div className={classNames("filters")} id="filters">
            <button className="filters__toggle mb-20">
      ...
      <div
        className={classNames("chat-tab", {
          "chat-tab--chat-open": isShowFilter,
        })}
        onClick={() => setIsShowFilter((prev: any) => !prev)}
      >
        <div className="chat-tab__center">
          <div className="icon-container">
            {isShowFilter ? <IconCloseCSGO /> : <IconChat />}
          </div>
        </div>
      </div>
      <div
        className={classNames("mask", { show: isOpenNavMobile })}
        onClick={() => {
          isOpenNavMobile && setOpenNavMobile(false);
        }}
      ></div>
      <div
        className={classNames("nav-tab", {
          "nav-tab--open": isOpenNavMobile,
        })}
        style={{ height: "100%" }}
      >
